...
export const InActiveBidding = ({ address, refreshBalance, assetDenomMap }) => {
  ...
  const [biddings, setInactiveBiddings] = useState("");
    ...
    queryDutchBiddingList(
      ...
      (error, result) => {
        ...
        if (result?.biddings?.length > 0) {
          ...
          setInactiveBiddings(reverseData);
