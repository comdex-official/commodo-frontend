// tf = range 60- 1H, 1440 - 1D, 10080 - 1W,  43800 - 1M
export const CAMPAIGN_URL = "https://test-campaign.comdex.one";
export const API_URL = process.env.REACT_APP_API_URL;
export const CSWAP_URL = process.env.REACT_APP_CSWAP_URL;
export const REWARDS_URL = process.env.REACT_APP_REWARDS_URL;
export const COINGECKO_API_URL = "https://api.coingecko.com/api/v3/simple/price?ids=comdex,cosmos,osmosis,axlusdc,axlweth,stride-staked-atom&vs_currencies=usd";